...
<div align="center">
	...
	<table class="table" style="text-align: center; border: 1px solid #dddddd; width: 70%;">
		<tr>
			...
			<td colspan="3">
				<input id="bname" value="<c:out value="${productDetail.bname }"/>" disabled="disabled">
			</td>
		</tr>
		<tr>
			<th style="background-color: #eeeeee; text-align: center;">상품명</th>
			<td colspan="3">
				<input id="pname" value="<c:out value="${productDetail.pname }"/>" disabled="disabled">
			</td>
		</tr>
		<tr>
			<th style="background-color: #eeeeee; text-align: center;">성별</th>
			<td>
				<c:if test="${productDetail.pGender==1 }">남성용</c:if>
				<c:if test="${productDetail.pGender==2 }">여성용</c:if>
				<c:if test="${productDetail.pGender==3 }">공용</c:if>
			</td>

			<th style="background-color: #eeeeee; text-align: center;">카테고리</th>
			<td>
				<c:if test="${productDetail.kind==1 }">상의</c:if>
				<c:if test="${productDetail.kind==2 }">하의</c:if>
				<c:if test="${productDetail.kind==3 }">잡화</c:if>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input name="balance" value="${productDetail.balance }" disabled="disabled">
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input name="price" value="${productDetail.price }" disabled="disabled">
				<span class="step_val"> 할인된 가격 (판매가) : <span class="span_discount"></span>
				</span>
			</td>

			<th style="background-color: #eeeeee; text-align: center; width: 200px">할인율</th>
			<td>
				<input name="discountrate_input" value="<fmt:formatNumber value="${productDetail.discountrate/100}" type="percent" />" disabled="disabled">
				<input type="hidden" name="discountrate" value="${productDetail.discountrate}" />
			</td>
		</tr>
		<tr>
