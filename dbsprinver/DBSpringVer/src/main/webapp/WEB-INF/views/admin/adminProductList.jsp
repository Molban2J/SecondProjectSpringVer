					...
					<tbody>
						<c:forEach items="${productList }" var="productList">
							<tr>
								...
								<td>
									<c:out value="${productList.num }"></c:out>
								</td>
								<td>
									<c:out value="${productList.bname }"></c:out>
								</td>
								<td>
									<a class="move" href='<c:out value="${productList.num }"/>'><c:out value="${productList.pname }"></c:out> </a>
								</td>
								<td>
									<c:if test="${productList.pGender==1 }">남자</c:if>
									<c:if test="${productList.pGender==2 }">여자</c:if>
									<c:if test="${productList.pGender==3 }">공용</c:if>
								</td>
								<td>
									<c:if test="${productList.kind==1 }">상의	</c:if>
									<c:if test="${productList.kind==2 }">하의	</c:if>
									<c:if test="${productList.kind==3 }">잡화	</c:if>
								</td>
								<td>
									<c:out value="${productList.psize }"></c:out>
								</td>
								<td>
									<c:out value="${productList.balance }"></c:out>
								</td>
								<td>
									<fmt:formatDate value="${productList.writedate  }" type="date" />
								</td>
							</tr>
						</c:forEach>
					</tbody>
				...
				<div class="pageMaker_wrap">
					...
					<ul class="pageMaker">
						...
						<c:if test="${pageMaker.prev}">
							<li class="pageMaker_btn prev"><a href="${pageMaker.startPage - 1}"><i class="fa fa-angles-left"></i></a></li>
						</c:if>
						...
						<c:if test="${pageMaker.next}">
							<li class="pageMaker_btn next"><a href="${pageMaker.endPage + 1 }"><i class="fa fa-angles-right"></i></a></li>
						</c:if>
				...
				<div class="search_wrap">
					<form id="searchForm" action="/admin/adminProductList" method="get">
						<div class="input-group" style="max-width: 400px;">
							<input type="text" placeholder="상품명을 입력 해주세요." class="form-control" name="keyword" style="width: 10%" value='<c:out value="${pageMaker.cri.keyword}"></c:out>'>
