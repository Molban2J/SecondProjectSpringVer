		...
		<form action="/admin/adminProductModify" method="post" id="modifyForm" enctype="multipart/form-data">
			...
			<table class="table" style="text-align: center; border: 1px solid #dddddd; width: 70%;">
				<tr>
					...
					<td colspan="4">
						<input id="bName_select" value="${productDetail.bname}" style="Width: 75%" readonly="readonly">
						<input id="bName_input" name="bname" type="hidden" value="${productDetail.bname}">
				...
				<tr>
					...
					<td colspan="4">
						<input id="pnameId" name="pname" value="${productDetail.pname }">
				...
				<tr>
					...
					<td colspan="4">
						<label>
							<input type="radio" name="pGender" value="1" ${productDetail.pGender == "1" ? "checked" : ""}>
							남성용
						</label>
						<label>
							<input type="radio" name="pGender" value="2" ${productDetail.pGender == "2" ? "checked" : ""}>
							여성용
						</label>
						<label>
							<input type="radio" name="pGender" value="3" ${productDetail.pGender == "3" ? "checked" : ""}>
							공용
						</label>
						<div>
				...
				<tr>
					...
					<td colspan="4">
						<label>
							<input type="radio" name="kind" value="1" ${productDetail.kind == "1" ? "checked" : ""}>
							상의
						</label>
						<label>
							<input type="radio" name="kind" value="2" ${productDetail.kind == "2" ? "checked" : ""}>
							하의
						</label>
						<label>
							<input type="radio" name="kind" value="3" ${productDetail.kind == "3" ? "checked" : ""}>
							잡화
						</label>
				...
				<tr>
					...
					<td>
						<input name="balance" type="number" value="${productDetail.balance }">
						<div>
				...
				<tr>
					...
					<td>
						<input name="price" type="number" value="${productDetail.price }">
						<div>
							<span class="step_val"> 할인된 가격 (판매가) : <span class="span_discount"></span>원
							</span> <span class="ck_warn price_warn">상품의 가격을 입력해주세요.</span>
						</div>
					</td>

					<th style="background-color: #eeeeee; text-align: center; width: 200px">할인율</th>
					<td>
						<input id="discount_interface" maxlength="2" value="<fmt:formatNumber value="${productDetail.discountrate/100}" type="percent" />">
						<input name="discountrate" type="hidden" value="0">
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="4">
						<c:choose>
							...
							<c:otherwise>
								<img src="/admin/display?fileName=${productDetail.imgUrl}" />
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
